...
fn selector(selector_str: &'_ str) -> scraper::Selector {
    let selector = scraper::Selector::parse(selector_str).unwrap();
    selector
}

/// Create a new selector by providing selector string.
fn try_selector(selector_str: &'_ str) -> Result<scraper::Selector, SelectorErrorKind<'_>>{
...
/// Retrieve the inner HTML of a document or fragment.
fn try_inner_html(document: scraper::html::Html, selector: scraper::Selector) -> Option<String> {
...
fn attr(document: scraper::html::Html, selector: scraper::Selector, attribute: &'_ str) -> String {
...
/// Retrieve the attribute of a document or selector.
fn try_attr(document: scraper::html::Html, selector: scraper::Selector, attribute: &'_ str) -> Option<String> {
...
fn text(document: scraper::html::Html, selector: scraper::Selector) -> String {
    let text = document.select(&selector).next().unwrap().text().collect::<String>();
    text
}

/// Retrieve text from a document or fragment.
fn try_text(document: scraper::html::Html, selector: scraper::Selector) -> Option<String> {
...
fn element<'a>(document: &'a scraper::html::Html, selector: scraper::Selector) -> scraper::ElementRef<'a> {
    let element_ref: scraper::ElementRef<'a> = document.select(&selector).next().unwrap();
    element_ref
}

fn try_element<'a>(document: &'a scraper::html::Html, selector: scraper::Selector) -> Option<scraper::ElementRef<'a>> {
